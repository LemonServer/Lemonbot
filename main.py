    ...
    try:
        while (True):
            if len(diagAll.GetChildren()[i].GetFirstChildControl().GetChildren()) == 3:
                ...
                diagAll.GetChildren()[i].Click()    # 选新消息的对话框
                if (WxRead()):
                    print("正在处理")
                    answer = zhipu_api.glm(Message[number[0]].MessageList)     # 调用api，给出ai的回答
                    print("Lemon bot answered a question")
                    print(Message[number[0]].username)
                    print(Message[number[0]].MessageList)
                    WxSend(Message[number[0]].username, answer)    # 发送给提问者
                    Message[number[0]].add_ai_answer(answer)
